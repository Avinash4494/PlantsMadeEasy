			...
			<div class="bodyContent">
     		...
     		<div class="col-lg-2">
     			<div class="headPaggignation">
     				<h5><a href="ordersPannel.php">Orders</a> / Pending Orders</h5>
     			</div>
     		</div>
     		<div class="col-lg-10">
...
<table id="example" cellpadding="0" cellspacing="0" border="0" class="datatable-1 table table-bordered table-striped	 display table-responsive" >
	...
	<tbody>
		<?php
		$statusDeli='Delivered';
			$query=mysqli_query($con,"select users.name as username,users.email as useremail,users.contactno as usercontact,users.shippingAddress as shippingaddress,users.shippingCity as shippingcity,users.shippingState as shippingstate,users.shippingPincode as shippingpincode,products.productName as productname,products.shippingCharge as shippingcharge,orders.quantity as quantity,
orders.productId as productid,products.productWeight as product_weight,
				orders.paymentMethod as payMethod,orders.orderStatus as order_status, products.pr_sellingPrice as pSell,orders.userOrderid as userorderid,
				orders.orderDate as orderdate,products.productPrice as productprice,orders.id as id  from orders join users on  orders.userId=users.id join products on products.id=orders.productId where orders.orderStatus!='$statusDeli' or orders.orderStatus is null");
		$cnt=1;
...
<?php } ?>
 ...
 <script>
  ...
  $('#json').on('click',function(){
    $("#example").tableHTMLExport({type:'json',filename:'PendingOrders.json'});
  })
  $('#csv').on('click',function(){
    $("#example").tableHTMLExport({type:'csv',filename:'PendingOrders.csv'});
  })
  </script>