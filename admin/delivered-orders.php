     ...
     <div class="row">
     		<div class="col-lg-2">
     			<div class="headPaggignation">
     				<h5><a href="ordersPannel.php">Orders</a> / Delivered Orders</h5>
     			</div>
     		</div>
     		<div class="col-lg-10">
...
<table id="example" cellpadding="0" cellspacing="0" border="0" class="datatable-1 table table-bordered table-striped display table-responsive" >
	...
	<tbody>
		...
		orders.productId as productid,products.productWeight as product_weight,
		 products.pr_sellingPrice as pSell,orders.userOrderid as userorderid,
						orders.paymentMethod as payMethod,orders.orderStatus as order_status,
			orders.quantity as quantity,orders.orderDate as orderdate,products.productPrice as productprice,orders.id as id  from orders join users on  orders.userId=users.id join products on products.id=orders.productId where orders.orderStatus='$st'");
		$cnt=1;
...
<script>
  $('#json').on('click',function(){
    $("#example").tableHTMLExport({type:'json',filename:'DeliveredOrders.json'});
  })
  $('#csv').on('click',function(){
    $("#example").tableHTMLExport({type:'csv',filename:'DeliveredOrders.csv'});
  })
  </script>
